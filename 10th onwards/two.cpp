  ...
  String html = R"rawliteral(
    ...
    <body>
      ...
      <script>
        ...
        if (key === 'w') fetch('/adjust?want_speed=inc');
        else if (key === 's') fetch('/adjust?want_speed=dec');
        else if (key === 'x') fetch('/adjust?want_speed=reset');
        else if (key === 'a') fetch('/set?turn=10');
        else if (key === 'd') fetch('/set?turn=-10');
        else if (key == 'f') fetch('/set?turn=0');
      });
