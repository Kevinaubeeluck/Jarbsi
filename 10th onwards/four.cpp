...
// handleset is to change the values of turn to 10, -10 or 0
void handleSet() {
...
// adjust is to change the value of "want_speed" to +0.01 when i press w and -0.01 when i press s
void handleAdjust() {
...
//hold ajust is when i hold it down it will constantly adjust between forward and still to keep it going forward at a steady speed and so it wont accelerate off
void handleHoldAdjust() {
  static unsigned long lastBlockTime = 0;
  static bool blocked = false;
  if (millis() - lastBlockTime < 300 && blocked) {
...
void loop()
{
  ...
  //update of c, d, e every 1 second
  ...
  // Clamping motorspeed
    ...
    step1.setAccelerationRad(accel+turn);
    step2.setAccelerationRad(accel-turn);
