...
void loop()
{
  ...
  if (millis() > loopTimer) {
    ...
    actual_motor_speed = step1.getSpeed();
    error_speed = motorspeed_setpoint-(actual_motor_speed/20000);
    float pre_angle_found = pid_2(error_speed);
    angle_found = pre_angle_found + 0.17;
