...
camera_config = picam2.create_video_configuration(
    ...
    controls={
        ...
        "ExposureTime": 10000, 
        "AeEnable": False # To control exposure time manually
    }
)
...
def send_numpy_frame(numpy_image, server_url):
...
def radial_expansion_normalized(matched1, matched2, optical_center):
    cx, cy = optical_center[1] , 0
    expansion_ratios = []

    for (x1, y1), (x2, y2) in zip(matched1, matched2):
        r1 = np.linalg.norm([x1 - cx, y1 - cy])
        r2 = np.linalg.norm([x2 - cx, y2 - cy])
        if r1 > 0:  # Avoid dividing by 0
    ...
    try:
        if queryDescriptors.shape[1] == trainDescriptors.shape[1]:  
            ...
            if np.array(matches).shape[1] == 2: 
                good_matches = []   # Lowe's Ratio Test
                for m, n in matches:
...
def get_P(m_frame1, m_frame2, camMat):
    ...
    try:
        if E.any() != None and E.shape==(3, 3):
            ...
            inlier_1 = m_frame1[mask.ravel() == 1] # Inlier Filtering
    ...
    except AttributeError: # Error Catching
        return False, np.hstack((np.eye(3), np.zeros((3, 1)))), m_frame1, m_frame2
...
def VO(matched_points_frame1, matched_points_frame2):
    ...
    if pts1.shape[0] > 2:
        ...
        points_4D = cv2.triangulatePoints(P1 , P2, np.array(inlier_1).T, np.array(inlier_2).T) # gives 4D homogenous coords of each point in frame 1 relative to frame 2
        points_3D = points_4D.T[:,0:3]/points_4D.T[:,3:]
        expansion_ratios = radial_expansion_normalized(inlier_1, inlier_2, (521, 796))
...
while True:
    ...
    if moving_forw:
        ...
        obst_found, points_3D, mask = VO(matched_points_frame1, matched_points_frame2)
        if obst_found:
            angles, C, R, L = seperate_by_angle(points_3D)
            print(f"L, C, R: {len(L)}, {len(C)}, {len(R)}")
            
            approaching_mask = np.array(angles[mask])
            center = len(approaching_mask[(approaching_mask.ravel() > -10) & (approaching_mask.ravel() < 10)])
            right = len(approaching_mask[approaching_mask.ravel() > 10])
            left = len(approaching_mask[approaching_mask.ravel() < -10])
