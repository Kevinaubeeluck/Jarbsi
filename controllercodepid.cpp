...
float pid(float tiltx){
  float error = (setpoint - tiltx)/1000;
  proportional = error;
  integral += error * dt;
...
void loop()
{
  ...
  if (millis() > loopTimer) {
    ...
    step1.setTargetSpeedRad(motorspeed/10);
    step2.setTargetSpeedRad(-motorspeed/10);
